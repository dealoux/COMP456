/* This program is a modified version of the expert shell as detailed in
    Chapter 6 of "AI Algorithms, Data Structures, and Idioms in Prolog, Lisp, and Java" by 
    George F. Luger and Willian A. Stubblefield
*/

/* Predefined prolog's predicates */
:- dynamic(known/2).
:- dynamic(assert/1).

/* Prompts user for knowledge base file */
run :-
	write('Please provide a knowledge base for the system.'), nl,
	read(Filename),
	consult(Filename).
...
respond(how(X), Goal, CF, Rules) :-
	...
	askuser(Goal, CF, Rules).
/* User enters a query, could not build proof */
respond(how(X), Goal, CF, Rules) :-
...
indent(0).
indent(l) :-
write(''), l_new is l - 1, indent(l_new).